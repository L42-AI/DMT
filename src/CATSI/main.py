...
class ContAwareTimeSeriesImp(object):
    def __init__(self, var_names,
        ...
        """ The first manipulation of the data happens in load_data. Returned objects are TimeSeriesDataSet objects,
        a custom Dataset subclass. The list of dictionaries (patients) is stored within self.content"""

        # create model
        self.device = device or torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.model = CATSI(len(var_names)).to(self.device)
        """ The only argument provided to construct CATSI object is the number of variables. RNNs can operate on variable
        time-sequences. """
    ...
    def fit(self, epochs=300, batch_size=64, eval_batch_size=64, eval_epoch=1, record_imp_epoch=50):
        ...
        """ These dictionaries seem to be optimization parameters for models in CATSI. Specifically the first one is for the Context RNN 
        (complex temporal dynamics) and the second one is for ALL other parameters of the model"""
        ...
        imp_dfs_train = None
        imp_dfs_valid = None
        """ This is where the imputation results will be held """
        ...
        for epoch in range(epochs):
            ...
            # evaluation loss on masked-out values
            total_loss_eval = AverageMeter()
            # individual loss components: training (rnn, feat, fusion) and eval (rnn, feat, fusion)
            verbose_loss = [AverageMeter() for _ in range(6)]

            for idx, data in enumerate(train_iter):
                """ Here the training algorithm iterates over batches in the training data. One iteration
                is one batch.
                """
                optimizer.zero_grad()
                """ This is to clear up the gradients from the previous batch"""
                ...
                """ Updates running average loss trackers 
                """
